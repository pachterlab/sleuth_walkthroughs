...
In addition to this walkthrough, data from the paper was is used for some of the analyses in the sleuth paper. 
...
Gene abundances can be viewed to confirm the differential analysis:

```{r}
plot_bootstrap(so,"ENSMUSG00000066553",units="scaled_reads_per_base",color_by="strain", x_axis_angle =50)
```

However visualization of gene abundances is not sufficient to fully quality control an experiment. A principal component analysis (PCA) of the kallisto abundances shows that while the samples appear to be clustered into a few distinct groups, they are not well-separated by strain.
