...
* Harold Pimentel, Nicolas L Bray, Suzette Puente, Páll Melsted and Lior Pachter, [Differential analysis of RNA-seq incorporating quantification uncertainty](), Nature Methods **14**, (2017), doi:
...
This walkthrough is based on the data in the "Cuffdiff2 paper":
...
To test for transcripts that are differential expressed between the conditions, sleuth performs a second fit to a model that presumes abundances are equal in the two conditions. 
...
The easiest way to view and interact with the results is to generate the Shiny webpage that allows for exploratory data analysis:

```{r eval=FALSE}
sleuth_live(so)
```

Among the tables and visualizations that can be explored with sleuth_live are a number of plots that provide an overview of the experiment. For example, a PCA plot of the samples:
```{r}
